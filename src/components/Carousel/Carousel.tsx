...
export default function Carousel() {
    ...
    useEffect(()=>{
        if(mainCarousel?.current){
            favorites.length == 0 ? mainCarousel.current.style.display = 'none' : mainCarousel.current.style.display = 'block'
        }
    },[favorites]);
            ...
            <motion.div className="carousel"  ref={carousel} whileTap={{cursor: "grabbing"}}>
                ...
                >
                    {favorites.map(favorite => (
                        <motion.div className="item-flex-container"  key={favorite.market_cap_rank.toString()}>
                            ...
                            <div className="flex-2">
                                <div className="flex-1">
                                    ...
                                    <img src={ checkHighLowCoin(favorite) ? iconPolygonHith : iconPolygonLow }></img>
                                </div>
                                <p className='current-price'>{ format(favorite.current_price)}</p>
