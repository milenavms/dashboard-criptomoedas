...
import { getListTopCriptomoedasService } from "services/getListTopCriptomoedasService";
...
export default function TableValues() {
  ...
  const [topCriptomoedas, setTopCriptomoedas] = React.useState<
    CriptoTopListInterface[]
  >([]);

  useEffect(() => {
    getListTopCriptomoedasService().then((response) =>
      setTopCriptomoedas(response)
    );
  }, []);
  ...
  const removeFavorites = (topCriptomoeda: CriptoTopListInterface) =>
    setFavorites(
      favorites.filter(
        (favorites) =>
          favorites.market_cap_rank != topCriptomoeda.market_cap_rank
      )
    );
                ...
                .map((topCriptomoeda) => {
                  return (
                    <TableRow
                      ...
                      role="checkbox"
                      tabIndex={-1}
                      key={topCriptomoeda.market_cap_rank}
                    >
                      <TableCell sx={{ width: "20px" }}>
                        <IconButton
                          onClick={() =>
                            checkIsFavorite(topCriptomoeda)
                              ? removeFavorites(topCriptomoeda)
                              : addFavorites(topCriptomoeda)
                          }
                        >
                      ...
                      <StyledTableCell align="left">
                        {topCriptomoeda.market_cap_rank}
                      </StyledTableCell>
                      <StyledTableCell
                        align="right"
                        sx={{ paddingRight: "0px" }}
                      ...
                      <StyledTableCell align="left">
                        ...
                        <span className="symbol">
                          {topCriptomoeda.symbol.toUpperCase()}
                        </span>
                      </StyledTableCell>
                      <StyledTableCell align="right">
                        {format(topCriptomoeda.current_price)}
                      </StyledTableCell>
                      <StyledTableCell align="right">
                        <div
                          className={
                            checkHighLowCoin24(topCriptomoeda)
                              ? "check-high"
                              : "check-low"
                          }
                        >
                          ...
                          />
                          {topCriptomoeda.price_change_percentage_24h.toFixed(
                            2
                          )}
                          %
                        </div>
                      ...
                      <StyledTableCell align="right">
                        <div
                          className={
                            checkHighLowCoin7(topCriptomoeda)
                              ? "check-high"
                              : "check-low"
                          }
                        >
                          ...
                          />
                          {topCriptomoeda.market_cap_change_percentage_24h.toFixed(
                            2
                          )}
                          %{" "}
                        </div>{" "}
                      ...
                      <StyledTableCell align="right">
                        {format(topCriptomoeda.market_cap)}{" "}
                      </StyledTableCell>
                    </TableRow>
                  );
                })}
